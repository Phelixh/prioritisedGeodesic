...
# I don't super care about the mice actually learning to make consistent choices
...
for session in range(num_sessions):
	...
	for trial in range(num_trials):
		...
		for adx, action in enumerate(act_seq):
			if action != 1:  # agent is making a choice, so do pre-choice replay
				prec_Gs[session, trial, :] = ga.G
				# replays, _, _ = ga.replay(num_replay_steps, goal_dist=softmax(replay_goal_vals, replay_temp), verbose=True,
				# 						  check_convergence=False, EVB_mode=replay_mode)
				# prechoice_replays[trial, :, :] = replays
...
# Save everything
np.savez('gillespie_session_data.npz', prec=prechoice_replays, posto=postoutcome_replays, state_trajs=states_visited,
		 choices=choices, rewards=rewards, goal_seq=goal_seq, prec_Gs=prec_Gs, posto_Gs=posto_Gs, allow_pickle=True)
