...
dist_subopt_bns = 3  # Distance to the two suboptimal bottlenecks
...
num_states += (dist_subopt_near - 1) + 2 * (dist_subopt_far - 1)  # Distance from one suboptimal bottleneck to its goals
num_states += (dist_subopt_far - 1) + 2 * (dist_subopt_near - 1)  # Distance from the other suboptimal bottleneck
...
lf_distances = [dist_subopt_near - 1, dist_subopt_far - 1, dist_subopt_far - 1]
for i in range(num_goals):
...
hf_distances = [dist_subopt_far - 1, dist_subopt_near - 1, dist_subopt_near - 1]
for i in range(num_goals):
...
s0_dist[0] = 1
sagiv_maze = DiGraph(num_states, edges, init_state_dist=s0_dist)
T = sagiv_maze.transitions
all_experiences = sagiv_maze.get_all_transitions()

sagiv_maze_wall = DiGraph(num_states, wall_edges, init_state_dist=s0_dist)
T_wall = sagiv_maze_wall.transitions
all_experiences_wall = sagiv_maze_wall.get_all_transitions()
...
for mouse in range(num_mice):
    ...
    for session in range(num_sessions):
        ...
        for trial in range(num_trials):
            ...
            #
            # prechoice_replays[mouse, session, trial, :, :] = replays
            ...
            while ga.curr_state not in goal_states:
                ...
                if trial == trials_before_wall and ga.curr_state == wall_loc and action == 3 and \
                    ...
                    # ga.G[wall_loc, action, :] = 0
                    # ga.G = np.zeros_like(ga.G)
                    ...
                    # DEBUG
                    for i in range(replays.shape[0]):
                        print(i, replays[i])
                    print('replay done')
            ...
            goal_dist = gdm @ goal_dist  # uhhh right?


# Save everything
np.savez('./Data/sagiv/sagiv_mouse_data.npz', posto=postoutcome_replays, prec=prechoice_replays,
